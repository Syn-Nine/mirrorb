...
pub struct World {
    ...
    pub gamepad_y: f32,
    rng: ThreadRng,
    pub mgui: std::boxed::Box<Mgui>,
    pub ugui: std::boxed::Box<Ugui>,
}
...
impl World {
    pub fn new(cache: &mut CacheManager) -> World {
        ...
        World {
            ...
            gamepad_y: 0.0,
            rng: rand::thread_rng(),
            mgui: Box::new(Mgui::new()),
            ugui: Box::new(Ugui::new()),
        }
    }
    ...
    pub fn entity_set_iso_tilemap(
        ...
        if 0 == columns { return; }
        self.tmcm.set_iso_tilemap(idx, tileset_idx, columns, data);
        self.ent.add_component(idx, COMPONENT_RENDER);
        self.rcm.set_type(idx, RENDER_TYPE_TILEMAP);
    }

    pub fn tilemap_cell_flip_horizontal(
        &mut self,
        idx: usize,
        cell: usize,
    ) {
        self.tmcm.cell_flip_horizontal(idx, cell);
    }

    pub fn tilemap_cell_flip_vertical(
        &mut self,
        idx: usize,
        cell: usize,
    ) {
        self.tmcm.cell_flip_vertical(idx, cell);
    }
    
    pub fn tilemap_cell_rotate_cw(
        &mut self,
        idx: usize,
        cell: usize,
    ) {
        self.tmcm.cell_rotate_cw(idx, cell);
    }

    pub fn tilemap_cell_rotate_ccw(
        &mut self,
        idx: usize,
        cell: usize,
    ) {
        self.tmcm.cell_rotate_ccw(idx, cell);
    }

    pub fn tilemap_cell_rotate_180(
        &mut self,
        idx: usize,
        cell: usize,
    ) {
        self.tmcm.cell_rotate_180(idx, cell);
    }
