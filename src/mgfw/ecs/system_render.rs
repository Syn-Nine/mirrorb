...
impl RenderSystem {
    ...
    pub fn render(&self, gl: &Gl, world: &mut World, start_time: std::time::Instant) {
            ...
            match rcm.get_type(i) {
                ...
                RENDER_TYPE_TILEMAP => {
                    if tmcm.is_constructed(i) {
                        ...
                        gl.draw_tilemap(
                            (pos.x + phys.velocity.x * dt).round(),
                            (pos.y + phys.velocity.y * dt).round(),
                            angle,
                            scale.x,
                            scale.y,
                            vao,
        ...
        // draw gui
        if world.mgui.is_constructed() {
            world.mgui.render(gl);
        }

        if world.ugui.is_constructed() {
            world.ugui.render(gl);
        }
